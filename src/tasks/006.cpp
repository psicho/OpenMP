// Задача 6
// Реализовать операцию редукции с использованием разных
// способов организации взаимоисключения (атомарные операции,
// критические секции, синхронизация при помощи замков и т.д.)
...
#pragma omp parallel for num_threads(30) default(shared) \
schedule(static,chunk) reduction(+:S)
for(i=0;i<n-1;i++)
{
  x=a+i*h;
  S=S+(1+sin(x)/(1+cos(x)));
